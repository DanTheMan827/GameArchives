...
namespace LibArchiveExplorer
{
    ...
    public partial class PackageView : UserControl
    {
        ...
        private async void ReplaceFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!pm.Ready)
            {
                MessageBox.Show("Please wait for the current operation to complete.");
            ...
            {
                pm.SetBusyState("Replacing file...");
                foreach (ListViewItem selItem in fileView.SelectedItems)
                {
                    if (selItem.Tag is IFile)
                        items.Add(selItem.Tag as IFile);
                }
            ...
            {
                if (items.Count > 1)
                    throw new Exception("More than one file selected");
                if (items.Count < 1)
                    throw new Exception("No files selected");

                using (FileStream replacement = File.OpenRead(fb.FileName))
                {
                    Stream ark;
                    var file = items[0] as GameArchives.Common.OffsetFile;

                    if (replacement.Length > file.Size)
                        throw new Exception("Replacement file is larger");

                    var multiStream = file.img_file as GameArchives.Common.MultiStream;
                    var offset = multiStream.offsetToStream(file.DataLocation, out ark);
                    
                    ark.Seek(offset, SeekOrigin.Begin);
                    await ark.WriteAsync(new byte[file.Size], 0, (int)file.Size);
                    ark.Seek(offset, SeekOrigin.Begin);
                    await replacement.CopyToAsync(ark);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error when replacing file:" + Environment.NewLine + ex.Message);
            }
            pm.SetReady();
        }
